...
(function () {
  ...
  function abandonIntro(playVoice) {
    clearBedTimers();
    introPlaying = false;
    introDone = true;
    document.body.classList.remove('is-intro');
  ...
  function skip(delta) {
    if (introPlaying) abandonIntro(true);
  ...
  function loadSpoken() {
    if (!spokenSrc) return;
    var jsonSrc = String(spokenSrc).replace(/\.spoken\.js(\?.*)?$/i, '.spoken.json$1');
    if (window.fetch && jsonSrc !== spokenSrc) {
      fetch(jsonSrc).then(function (r) {
        if (!r.ok) throw new Error('spoken json');
        return r.json();
      }).then(applySpokenData).catch(function () {
        loadSpokenScript(spokenSrc);
      });
      return;
    }
    loadSpokenScript(spokenSrc);
  ...
  function seekToMark() {
    if (introPlaying) abandonIntro(false);
    if (!audio.duration) return;
  ...
  audio.addEventListener('timeupdate', function () {
    ...
    if (wrap && audio.duration) {
      wrap.setAttribute('aria-valuemax', String(Math.floor(audio.duration)));
      wrap.setAttribute('aria-valuenow', String(Math.floor(audio.currentTime || 0)));
    }
    syncLine();
    checkSleep();
